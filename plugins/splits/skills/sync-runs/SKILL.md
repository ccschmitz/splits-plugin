...
2. Call Splits `get_sync_context` for that provider and range. If a connector needs authentication, ask the athlete to connect it. If Splits reports a source conflict, stop before provider discovery. State which provider is selected and that no activities were imported. Do not ask whether to switch or imply this workflow can switch sources. Explain that a source change requires an explicit action in Splits Account and that only a provider connected directly to Splits can be selected there; an assistant-side connector alone is not selectable.
...
6. Call `get_sync_context` again for the same provider and range. Report imported, updated, already present, matched, unmatched, and failed results from the Splits receipt. Distinguish a saved activity from a matched workout and name any truncated discovery or remaining failure.
