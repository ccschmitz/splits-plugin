...
If the athlete asks to fetch or sync runs from a connected provider, use `sync-runs`; that workflow resolves whether the provider has a supported adapter. Do not ask them to manufacture an export or reconstruct provider rows here.
...
Before a vendor export import, call `get_sync_context` with its provider and local dates (split windows longer than 31 dates). When `approvalRequired` is true, explain which provider and supplied runs will be imported, which source will remain primary, and that primary duplicates will be skipped. Ask for the athlete's approval of this specific secondary import before saving. The returned `secondarySourceApproval` object is a description of the scope, not evidence of approval: pass it to `import_activities` only after the athlete agrees. Reuse that approval for batches/retries within the same requested scope; never carry it to a later request. A decline saves nothing. If the primary or scope changes, refresh context and approval. No Account setting or primary-source switch is needed. On a server that lacks this contract, report the limitation instead of bypassing it. Described manual runs use the existing logging flow below.
